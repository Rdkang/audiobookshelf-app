<template>
  <div ref="card" :id="`series-card-${index}`" :style="{ width: width + 'px', height: height + 'px' }" class="rounded-shape-corner-medium cursor-pointer z-30 bg-md-sys-color-surface-container shadow-elevation-1 hover:shadow-elevation-2 material-you-transition" @click="clickCard">
    <div class="w-full h-full relative rounded-shape-corner-medium overflow-hidden">
    ...
    <div v-if="seriesPercentInProgress > 0" class="absolute bottom-0 left-0 h-1 max-w-full z-10 rounded-b w-full shadow-elevation-2" :class="isSeriesFinished ? 'bg-md-sys-color-primary' : 'bg-md-sys-color-secondary'" :style="{ width: seriesPercentInProgress * 100 + '%' }" />
