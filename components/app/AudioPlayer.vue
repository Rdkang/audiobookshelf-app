<template>
  <div v-if="playbackSession" id="streamContainer" class="fixed top-0 left-0 layout-wrapper right-0 z-50 pointer-events-none" :class="{ fullscreen: showFullscreen, 'ios-player': $platform === 'ios', 'web-player': $platform === 'web' }">
    <div v-if="showFullscreen" class="w-full h-full z-10 absolute top-0 left-0 pointer-events-auto bg-md-sys-color-surface-container" :style="{ backgroundColor: coverRgb }">
      <div class="w-full h-full absolute top-0 left-0 pointer-events-none bg-gradient-to-b from-md-sys-color-scrim/20 to-md-sys-color-scrim/60" />

      <div class="top-6 left-6 absolute cursor-pointer material-you-state-layer rounded-full p-2">
        <span class="material-symbols text-4xl text-md-sys-color-on-surface" :class="{ 'text-black text-opacity-75': coverBgIsLight }" @click="collapseFullscreen">keyboard_arrow_down</span>
      </div>
      <div v-show="showCastBtn" class="top-6 right-20 absolute cursor-pointer material-you-state-layer rounded-full p-2">
        <span class="material-symbols text-3xl text-md-sys-color-on-surface" :class="coverBgIsLight ? 'text-black' : ''" @click="castClick">{{ isCasting ? 'cast_connected' : 'cast' }}</span>
      </div>
      <div class="top-6 right-6 absolute cursor-pointer material-you-state-layer rounded-full p-2">
        <span class="material-symbols text-3xl text-md-sys-color-on-surface" :class="{ 'text-black text-opacity-75': coverBgIsLight }" @click="showMoreMenuDialog = true">more_vert</span>
      </div>
      <p class="top-16 absolute left-0 right-0 mx-auto text-center uppercase tracking-widest text-md-sys-color-on-surface-variant" :class="{ 'text-black text-opacity-75': coverBgIsLight }" style="font-size: 10px">{{ isDirectPlayMethod ? $strings.LabelPlaybackDirect : isLocalPlayMethod ? $strings.LabelPlaybackLocal : $strings.LabelPlaybackTranscode }}</p>
    </div>
    ...
    <div v-if="playerSettings.useChapterTrack && playerSettings.useTotalTrack && showFullscreen" class="absolute total-track w-full z-30 px-6">
      <div class="flex">
        <p class="font-mono text-md-sys-color-on-surface" style="font-size: 0.8rem">{{ currentTimePretty }}</p>
        <div class="flex-grow" />
        <p class="font-mono text-md-sys-color-on-surface" style="font-size: 0.8rem">{{ totalTimeRemainingPretty }}</p>
      </div>
      <div class="w-full">
        <div class="h-1 w-full bg-md-sys-color-outline-variant/50 relative rounded-full">
          <div ref="totalReadyTrack" class="h-full bg-md-sys-color-surface-container-high absolute top-0 left-0 pointer-events-none rounded-full" />
          <div ref="totalBufferedTrack" class="h-full bg-md-sys-color-outline absolute top-0 left-0 pointer-events-none rounded-full" />
          <div ref="totalPlayedTrack" class="h-full bg-md-sys-color-primary absolute top-0 left-0 pointer-events-none rounded-full" />
        </div>
      </div>
    </div>
    ...
    <div class="title-author-texts absolute z-30 left-0 right-0 overflow-hidden" @click="clickTitleAndAuthor">
      ...
      <p class="author-text text-md-sys-color-on-surface-variant truncate">{{ authorName }}</p>
    </div>
    ...
    <div id="playerContent" class="playerContainer w-full z-20 absolute bottom-0 left-0 right-0 p-2 pointer-events-auto transition-all" :style="{ backgroundColor: showFullscreen ? '' : coverRgb }" @click="clickContainer">
      <div v-if="showFullscreen" class="absolute bottom-4 left-0 right-0 w-full pb-4 pt-2 mx-auto px-6" style="max-width: 414px">
        <div class="flex items-center justify-between pointer-events-auto">
          <span v-if="!isPodcast && serverLibraryItemId && socketConnected" class="material-symbols text-3xl text-md-sys-color-on-surface-variant cursor-pointer material-you-state-layer rounded-full p-2" :class="{ fill: bookmarks.length }" @click="$emit('showBookmarks')">bookmark</span>
          <!-- hidden for podcasts but still using this as a placeholder -->
          <span v-else class="material-symbols text-3xl text-white text-opacity-0">bookmark</span>

          <span class="font-mono text-md-sys-color-on-surface-variant cursor-pointer material-you-state-layer rounded-full px-3 py-2" style="font-size: 1.35rem" @click="$emit('selectPlaybackSpeed')">{{ currentPlaybackRate }}x</span>
          <svg v-if="!sleepTimerRunning" xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-md-sys-color-on-surface-variant cursor-pointer material-you-state-layer rounded-full p-2 box-content" fill="none" viewBox="0 0 24 24" stroke="currentColor" @click.stop="$emit('showSleepTimer')">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
          </svg>
          <div v-else class="h-7 w-7 flex items-center justify-around cursor-pointer material-you-state-layer rounded-full p-2 box-content" @click.stop="$emit('showSleepTimer')">
            <p class="text-xl font-mono text-md-sys-color-tertiary">{{ sleepTimeRemainingPretty }}</p>
          </div>

          <span class="material-symbols text-3xl text-md-sys-color-on-surface cursor-pointer material-you-state-layer rounded-full p-2" :class="chapters.length ? 'text-opacity-75' : 'text-opacity-10'" @click="clickChaptersBtn">format_list_bulleted</span>
        </div>
      </div>
      <div v-else class="w-full h-full absolute top-0 left-0 pointer-events-none bg-gradient-to-t from-md-sys-color-scrim/60 to-md-sys-color-scrim/20" />

      <div id="playerControls" class="absolute right-0 bottom-0 mx-auto" style="max-width: 414px">
        <div class="flex items-center max-w-full" :class="playerSettings.lockUi ? 'justify-center' : 'justify-between'">
          <span v-show="showFullscreen && !playerSettings.lockUi" class="material-symbols next-icon text-md-sys-color-on-surface cursor-pointer material-you-state-layer rounded-full p-2" :class="isLoading ? 'text-opacity-10' : 'text-opacity-75'" @click.stop="jumpChapterStart">first_page</span>
          <span v-show="!playerSettings.lockUi" class="material-symbols jump-icon text-md-sys-color-on-surface cursor-pointer material-you-state-layer rounded-full p-2" :class="isLoading ? 'text-opacity-10' : 'text-opacity-75'" @click.stop="jumpBackwards">{{ jumpBackwardsIcon }}</span>
          <div class="play-btn cursor-pointer shadow-elevation-2 flex items-center justify-center rounded-full text-primary mx-4 relative overflow-hidden material-you-state-layer" :style="{ backgroundColor: coverRgb }" :class="{ 'animate-spin': seekLoading }" @mousedown.prevent @mouseup.prevent @click.stop="playPauseClick">
          ...
          <span v-show="!playerSettings.lockUi" class="material-symbols jump-icon text-md-sys-color-on-surface cursor-pointer material-you-state-layer rounded-full p-2" :class="isLoading ? 'text-opacity-10' : 'text-opacity-75'" @click.stop="jumpForward">{{ jumpForwardIcon }}</span>
          <span v-show="showFullscreen && !playerSettings.lockUi" class="material-symbols next-icon text-md-sys-color-on-surface cursor-pointer material-you-state-layer rounded-full p-2" :class="nextChapter && !isLoading ? 'text-opacity-75' : 'text-opacity-10'" @click.stop="jumpNextChapter">last_page</span>
        </div>
      </div>

      <div id="playerTrack" class="absolute left-0 w-full px-6">
        <div class="flex pointer-events-none">
          <p class="font-mono text-md-sys-color-on-surface" style="font-size: 0.8rem" ref="currentTimestamp">0:00</p>
          <div class="flex-grow" />
          <p class="font-mono text-md-sys-color-on-surface" style="font-size: 0.8rem">{{ timeRemainingPretty }}</p>
        </div>
        <div ref="track" class="h-1.5 w-full bg-track/50 relative rounded-full" :class="{ 'animate-pulse': isLoading }" @click.stop>
          ...
          <div ref="trackCursor" class="h-7 w-7 rounded-full absolute pointer-events-auto flex items-center justify-center material-you-state-layer" :style="{ top: '-11px' }" :class="{ 'opacity-0': playerSettings.lockUi || !showFullscreen }" @touchstart="touchstartCursor">
