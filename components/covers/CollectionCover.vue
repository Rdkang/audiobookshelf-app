<template>
  <div class="relative rounded-sm overflow-hidden" :style="{ width: width + 'px', height: height + 'px' }">
    <div v-if="hasOwnCover" class="w-full h-full relative rounded-sm">
      <div v-if="showCoverBg" class="bg-md-sys-color-surface-container absolute top-0 left-0 w-full h-full">
    ...
    <div v-else-if="books.length" class="flex justify-center h-full relative bg-md-sys-color-surface-container bg-opacity-95 rounded-sm">
    ...
    <div v-else class="relative w-full h-full flex items-center justify-center p-2 bg-md-sys-color-surface-container rounded-sm">
      <div class="absolute top-0 left-0 w-full h-full bg-gray-400 bg-opacity-5" />

      <p class="text-md-sys-color-on-surface-variant text-center" :style="{ fontSize: Math.min(1, sizeMultiplier) + 'rem' }">Empty Collection</p>
    </div>
  </div>
</template>
