<template>
  <div class="w-full h-full py-6 px-4 overflow-y-auto">
    <p class="mb-2 text-base text-md-sys-color-on-surface">{{ $strings.HeaderDownloads }} ({{ localLibraryItems.length }})</p>

    <div class="w-full">
      <div v-for="(mediaItem, num) in localLibraryItems" :key="mediaItem.id" class="w-full">
        <nuxt-link :to="`/localMedia/item/${mediaItem.id}`" class="flex items-center">
          <div class="w-16 h-16 min-w-16 min-h-16 flex-none bg-md-sys-color-surface-container relative">
          ...
          <div class="px-2 flex-grow">
            ...
            <p v-if="mediaItem.mediaType == 'book'" class="text-xs text-md-sys-color-on-surface-variant">{{ mediaItem.media.tracks.length }} {{ $strings.LabelTracks }}</p>
            <p v-else-if="mediaItem.mediaType == 'podcast'" class="text-xs text-md-sys-color-on-surface-variant">{{ mediaItem.media.episodes.length }} {{ $strings.HeaderEpisodes }}</p>
            <p v-if="mediaItem.size" class="text-xs text-md-sys-color-on-surface-variant">{{ $bytesPretty(mediaItem.size) }}</p>
          </div>
          <div class="w-12 h-12 flex items-center justify-center">
            <span class="material-symbols text-2xl text-md-sys-color-on-surface-variant">chevron_right</span>
          </div>
        </nuxt-link>
